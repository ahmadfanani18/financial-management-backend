...
export class BudgetService {
    ...
    async updateSpent(id, userId, spent) {
    ...
    async getSpending(userId, budgetId) {
    ...
    async getAllWithSpending(userId, month) {
        ...
        const results = await Promise.all(filteredBudgets.map(async (budget) => {
            const startDate = new Date(budget.startDate);
            const endDate = budget.endDate || new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0);
            const transactions = await prisma.transaction.findMany({
                where: {
