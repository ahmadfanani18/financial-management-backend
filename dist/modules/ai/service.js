...
function weightedAverage(amounts) {
...
function calculateTimeSpanMonths(transactions) {
...
function calculateConfidenceScore(transactionCount, timeSpanMonths, lookbackMonths) {
...
export class AIService {
    ...
    generateDynamicMilestones(monthlyIncome, estimatedExpense, dependents) {
        const milestones = [];
        const now = Date.now();
    ...
    async predictSpending(userId, input) {
        const { months } = input;
    ...
    async generatePlanFromData(userId) {
        const accounts = await prisma.account.findMany({
            where: { userId, isArchived: false },
