...
export async function getMonthlyReportHandler(request, reply) {
...
export async function getCategoryBreakdownHandler(request, reply) {
...
export async function getNetWorthHandler(request, reply) {
    const netWorth = await reportService.getNetWorth(request.user.id);
    return reply.send(netWorth);
}
export async function getMutationsHandler(request, reply) {
...
export async function exportMutationsHandler(request, reply) {
    const query = mutationsQuerySchema.parse(request.query);
...
export async function exportTransactionsHandler(request, reply) {
    const { year, month } = monthlyReportSchema.parse(request.query);
    const csv = await reportService.exportTransactions(request.user.id, Number(year), Number(month));
