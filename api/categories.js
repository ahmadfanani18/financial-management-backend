import { getPrisma, parseBody, setupCors, parseToken } from './utils.js';

export default async function handler(req, res) {
  let db = null;
  try {
    const origin = req.headers.origin;
    setupCors(res, origin);

    if (req.method === 'OPTIONS') {
      res.status(204).send('');
      return;
    }

    res.setHeader('Content-Type', 'application/json');
    const url = (req.url || '/').split('?')[0];
    const method = req.method;

    const token = parseToken(req.headers.authorization);
    if (!token) {
      res.status(401).send(JSON.stringify({ message: 'Unauthorized' }));
      return;
    }

    db = await getPrisma();

    // GET all categories
    if (url === '/api/categories' && method === 'GET') {
      const queryParams = new URL(req.url, 'http://localhost').searchParams;
      const type = queryParams.get('type');
      const where = { userId: token.userId };
      if (type && ['INCOME', 'EXPENSE'].includes(type)) {
        where.type = type;
      }
      const categories = await db.category.findMany({
        where,
        orderBy: type === 'INCOME' ? { name: 'asc' } : { createdAt: 'desc' },
      });
      res.status(200).send(JSON.stringify({ categories }));
      return;
    }

    // POST create category
    if (url === '/api/categories' && method === 'POST') {
      const body = parseBody(req.body);
      const icon = body.name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
      const category = await db.category.create({
        data: {
          userId: token.userId,
          name: body.name,
          type: body.type || 'EXPENSE',
          icon: body.icon || icon,
          color: body.color || '#3B82F6',
          isDefault: false,
        },
      });
      res.status(201).send(JSON.stringify({ category }));
      return;
    }

    // GET category by ID
    const categoryMatch = url.match(/^\/api\/categories\/([a-f0-9-]+)$/i);
    if (categoryMatch && method === 'GET') {
      const category = await db.category.findFirst({ where: { id: categoryMatch[1], userId: token.userId } });
      if (!category) {
        res.status(404).send(JSON.stringify({ message: 'Kategori tidak ditemukan' }));
        return;
      }
      res.status(200).send(JSON.stringify({ category }));
      return;
    }

    // PUT update category
    if (categoryMatch && method === 'PUT') {
      const body = parseBody(req.body);
      const existing = await db.category.findFirst({ where: { id: categoryMatch[1], userId: token.userId } });
      if (!existing) {
        res.status(404).send(JSON.stringify({ message: 'Kategori tidak ditemukan' }));
        return;
      }
      const category = await db.category.update({
        where: { id: categoryMatch[1] },
        data: body,
      });
      res.status(200).send(JSON.stringify({ category }));
      return;
    }

    // DELETE category
    if (categoryMatch && method === 'DELETE') {
      const existing = await db.category.findFirst({ where: { id: categoryMatch[1], userId: token.userId } });
      if (!existing) {
        res.status(404).send(JSON.stringify({ message: 'Kategori tidak ditemukan' }));
        return;
      }
      if (existing.isDefault) {
        res.status(400).send(JSON.stringify({ message: 'Tidak dapat menghapus kategori default' }));
        return;
      }
      await db.category.delete({ where: { id: categoryMatch[1] } });
      res.status(204).send(JSON.stringify({ message: 'Deleted' }));
      return;
    }

    res.status(404).send(JSON.stringify({ error: 'Not found', url, method }));
  } catch (err) {
    console.error('Categories handler error:', err);
    res.status(500).send(JSON.stringify({ message: 'Internal server error', error: String(err) }));
  } finally {
    if (db) {
      try { await db.$disconnect(); } catch {}
    }
  }
}