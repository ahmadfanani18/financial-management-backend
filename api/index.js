...
export default async function handler(req, res) {
  ...
  try {
    ...
    const singleMilestoneMatch = url.match(/^\/api\/plans\/([a-f0-9-]+)\/milestones\/([a-f0-9-]+)$/i);
    if (singleMilestoneMatch && method === 'PUT') {
      const planId = singleMilestoneMatch[1];
      const milestoneId = singleMilestoneMatch[2];
      const body = parseBody(req.body);
    ...
    const completeMilestoneMatch = url.match(/^\/api\/plans\/([a-f0-9-]+)\/milestones\/([a-f0-9-]+)\/complete$/i);
    if (completeMilestoneMatch && method === 'PATCH') {
      const planId = completeMilestoneMatch[1];
      const milestoneId = completeMilestoneMatch[2];
      const milestone = await db.planMilestone.update({
        where: { id: milestoneId },
