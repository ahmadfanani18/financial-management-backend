...
export default async function handler(req, res) {
  ...
  if (url === '/api/reports/monthly' && method === 'GET') {
    ...
    const totalTransfer = transactions.filter(t => t.type === 'TRANSFER').reduce((sum, t) => sum + Number(t.amount), 0);
    res.status(200).send(JSON.stringify({
      report: {
        period: { year, month, label: `${month}/${year}` },
        summary: { totalIncome, totalExpense, totalTransfer, balance: totalIncome - totalExpense },
        incomeByCategory: [],
        expenseByCategory: [],
        transactions
      }
  ...
  if (url === '/api/reports/trends' && method === 'GET') {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      trends.push({
        month: new Date(year, month).toLocaleString('id-ID', { month: 'short' }),
        year,
        income,
        expense,
        transfer,
